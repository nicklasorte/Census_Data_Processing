...
'Need to rescrap with the water'

data_header='pop3water'
...
%         temp_state_block_data=cell_block_group_data{sub_idx,2};
%         size(temp_state_block_data)
%         water_idx=find(temp_state_block_data(:,9)>3*10^7);
%         size(water_idx)
% 
%         close all;
%         figure;
%         hold on;
%         plot(temp_state_block_data(water_idx,8),temp_state_block_data(water_idx,7),'ob')
%         grid on;
%         plot_google_map('maptype','terrain','APIKey','AIzaSyCgnWnM3NMYbWe7N4svoOXE7B2jwIv28F8') %%%Google's API key made by nick.matlab.error@gmail.com
% 
% figure;
% histogram(temp_state_block_data(:,9))
% set(gca, 'YScale', 'log')
%         min(temp_state_block_data(:,9))
%         max(temp_state_block_data(:,9))
% 
...
'now go back and integrate the water data into the microcell'
pause;
...
if var_exist_group_cell==2
    ...
    for state_idx=1:1:num_states
        ...
        if block_state==9  %%%%%Connecticut
            ...
            cell_block_group_data{state_idx,3}=temp_cell_block_group_ua;
...
'Use urban area codes in the microcell'
