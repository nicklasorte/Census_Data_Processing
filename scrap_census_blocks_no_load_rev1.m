function []=scrap_census_blocks_no_load_rev1(app,shape_folder,tf_rescrap_shape,str_header_keep,str_header_array_keep1,str_header_array_keep2)
   ...
   %%%%%%%%%%Function to go to the folder and pull the shp file. Save as mat file.
    ...
    if var_exist_cell_mat==2 && var_exist_array_mat==2
        ...
        tic;
        save(filename_cell_shape_mat,'cell_census_data')  %%%%%%%7MBs
        toc;

        %%%%%%%%Just the numbers?
        %%%%%%%Only keep this data
        num_keep2=length(str_header_array_keep1);
        keep2_idx=NaN(num_keep2,1);
        for col_idx=1:1:num_keep2
            keep2_idx(col_idx)=find(matches(struct_names,str_header_array_keep1{col_idx}));
        end

        num_keep3=length(str_header_array_keep2);
        keep3_idx=NaN(num_keep3,1);
        for col_idx=1:1:num_keep3
            keep3_idx(col_idx)=find(matches(struct_names,str_header_array_keep2{col_idx}));
        end
        tic;
        array_census_data=horzcat(str2double(shape_cell(keep2_idx,:)'),cell2mat(shape_cell(keep3_idx,:)'));
        toc;
        array_census_data(1:10,:)
        tic;
